...
export default function Login() {
  const { setAuth, clearHistory, setLang, lang } = useTwinStore();
  ...
  const handleSuccess = (userId: string, accessToken: string) => {
    // حفظ التوكن فوراً
    setToken(accessToken);
    // مسح المحادثة القديمة
    clearHistory();
    // تسجيل المستخدم
    setAuth(userId);
    router.replace('/chat');
    ...
    try {
      ...
      if (data?.url) {
        ...
        if (result.type === 'success') {
          ...
          if (session) handleSuccess(session.user.id, session.access_token);
        }
      }
    } catch (e: any) {
  ...
  const signInWithEmail = async () => {
    ...
    try {
      ...
      Alert.alert(isAr ? 'خطأ' : 'Error', isAr ? 'تعذر الاتصال' : 'Connection failed');
    } finally { setLoading(false); }
  };
  ...
  const signUpWithEmail = async () => {
    ...
    try {
      ...
      if (error) Alert.alert(isAr ? 'خطأ' : 'Error', error.message);
        ...
        // لو Supabase أرسل session مباشرة
        handleSuccess(data.user!.id, data.session.access_token);
      } else {
        Alert.alert(isAr ? 'تم ✅' : 'Done ✅', isAr ? 'تم إرسال رابط التأكيد لبريدك' : 'Confirmation link sent');
      }
    } catch {
      Alert.alert(isAr ? 'خطأ' : 'Error', isAr ? 'تعذر الاتصال' : 'Connection failed');
    } finally { setLoading(false); }
